...
#if USE_RCS_ID
const char uucico_rcsid[] = "$Id: uucico.c,v 1.140 1993/01/18 05:03:06 ian Rel $";
#endif
...
/* For COHERENT, '-c system' means call the system only if there is
 * work pending. This is to maintain backward compatibility with earlier
 * versions of Coherent 4.x uucp.
 */

#if defined(__COHERENT__)
  while ((iopt = getopt_long (argc, argv,
			      "c:DefI:lp:qr:s:S:u:x:X:w",
...
#if defined(__COHERENT__)
	case 'c':
		/* for COHERENT backward compatibility, -c and -s are 
		 * almost synonomous.
		 */
		checkfirst = TRUE;
		zsystem = optarg;
...
#if defined(__COHERENT__) /* backward compatibility with older COH -c option */
	if((checkfirst && fsysdep_has_work(&sLocked_system)) || (!checkfirst)){
#endif /* COHERENT */
...
#if defined(__COHERENT__) /* backward compat. with older COH -c option */
	}
#endif /* COHERENT */
