...
buildTab()
{
	...
	/* output hash table */
	fprintf(ofp, "KEYTAB keytab[] = {\n");
	for(i = 0; i < l; i++) {
		if(-1 == (k = keytab + i)->k_code)
			fprintf(ofp, "{\t-1, 0, -1},\t/* hole in table */\n");
		else {
			p = ctlName + k->k_code;
			fprintf(ofp, "{%16s, %3d, %3d },\t/* %-16s %d */\n",
...
main()
{
	...
	fprintf(ofp, "/*\n");
	fprintf(ofp, " * Emacs command tables.\n");
	fprintf(ofp, " * Generated by ext.c\n");
	fprintf(ofp, " * Make any changes by changing ext.c\n");
	fprintf(ofp, " */\n\n");
	fprintf(ofp, "#include \"stdio.h\"\n");
	fprintf(ofp, "#include \"ed.h\"\n\n");
	for(p = nameCom; NULL != *p; p += 2)
		if(**p != '*')
			fprintf(ofp, "extern %16s();\t/*%s*/\n", p[0], p[1]);
		else
			fprintf(ofp, "\t\t\t\t/*%s*/\n", p[1]);
