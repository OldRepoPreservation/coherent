...
#define DEF_BLOCKING	1	/* default blocking factor for extract */
...
#endif
{
    ...
    /* Parse options */
    while ((c = taropt(argc, argv, "b:cf:hlmortvwx")) != EOF) {
	switch (c) {
    ...
    if (f_create + f_extract + f_list + f_append != 1) {
	(void) fprintf(stderr,
	   "%s: you must specify exactly one of the c, t, r, or x options\n",
		       myname);
	usage();
	exit(EX_ARGSBAD);
    }
